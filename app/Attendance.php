...
class Attendance extends Model
{
        ...
        foreach ($att_records as $att_record) {
            ...
            $shift_detail = DB::table('employeeshiftdetails')
                ...
                if ($shift_detail) {
                    ...
                    $otcaltype = $att_record->ot_calculate_type;
                    $otcaltime = $att_record->ot_calculate_time;
                }

            $ot_hours = (new \App\Attendance)->get_ot_hours_by_date($att_record->uid, $off_time, $on_time, $record_date, $on_duty_time, $off_duty_time, $att_record->emp_department, $otcaltype, $otcaltime);
    ...
    public function get_ot_hours_by_date($emp_id, $off_time, $on_time, $record_date, $shift_start_, $shift_end_, $emp_department, $otcaltype, $otcaltime){
        $off_time = Carbon::parse($off_time);
        $on_time = Carbon::parse($on_time);
        $record_date = Carbon::parse($record_date);  
        $workdate = $record_date->format('Y-m-d');      
        ...
        $shift = DB::table('employeeshiftdetails')
            ->leftjoin('shift_types', 'employeeshiftdetails.shift_id', '=', 'shift_types.id')
            ->select('shift_types.*') 
            ->where('employeeshiftdetails.emp_id', $emp_id)
            ->whereDate('employeeshiftdetails.date_from', '<=', $workdate)
            ->whereDate('employeeshiftdetails.date_to', '>=', $workdate)
            ->first();
        ...
        //Night OT Allocation Check 2025-03-21
        $checknightotrecord  = DB::table('ot_allocation')
        ->select('ot_allocation.date', 'ot_allocation.time_from', 'ot_allocation.time_to') 
        ->whereDate('.ot_allocation.date', $workdate)
        ->first();

        $nightshiftallow = DB::table('ot_allocationdetails')
            ->leftjoin('ot_allocation', 'ot_allocationdetails.ot_allocation_id', '=', 'ot_allocation.id')
            ->select('ot_allocationdetails.time_from', 'ot_allocationdetails.time_to') 
            ->where('ot_allocationdetails.emp_id', $emp_id)
            ->whereDate('ot_allocation.date', $workdate)
            ->first();

        if(!empty($nightshiftallow)){
            if($nightshiftallow->time_from <= $off_time && $nightshiftallow->time_to >= $off_time){
                $otallowstatus=1;
            }
            else{
                $otoverstatus=1;
        ...
        else if(!empty($checknightotrecord)){
            if($checknightotrecord->date == $workdate && $checknightotrecord->time_from <= $off_time && $checknightotrecord->time_to >= $off_time){
                $otnotallowstatus=1;
            }
        }
        //Night OT Allocation Check 2025-03-21
        ...
        if ($offdutyTime->lessThan($ondutyTime)) {
            $offdutyTime->addDay();
        }
            ...
            if($date_period == 0){
                    ...
                    if($holiday_check->work_level==1){
                        if($holiday_check->holiday_type==1){
                            if($ot_minutes >= $otminimumminits){
                                ...
                                // $poya_work_days += 1;
                                $poya_work_days += round(min($holidayot / 8, 1), 2);
                            }
                        }
                        else{
                            if($ot_minutes >= $otminimumminits){
                                ...
                                if($double_ot_hours>8){
                                    $triple_ot_hours = $double_ot_hours-8;
                                    $double_ot_hours = 0;
                                    $holidaydouble = 8;
                                }
                                else{
                                ...
                                $mercantile_work_days += round(min($holidaydouble / 8, 1), 2);
                            }
                        }                        
                    }
                    else{
                ...
                if($ot_hours_morning>0 | $double_ot_hours_morning>0 | $triple_ot_hours_morning>0){
                    $ob = array(
                        ...
                        'otallowstatus' => $otallowstatus,
                        'otoverstatus' => $otoverstatus,
                        'otnotallowstatus' => $otnotallowstatus,
                    );
