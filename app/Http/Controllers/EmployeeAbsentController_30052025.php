...
class EmployeeAbsentController extends Controller
{
    ...
    public function get_absent_employees(Request $request)
    {
        ...
        if($department=='All'){
            ...
            ->select('employees.emp_id', 'employees.emp_name_with_initial','employees.emp_department','employees.emp_address','employees.emp_mobile','departments.name AS departmentname','branches.location AS location') 
            ->where('deleted', 0)
            ->where('is_resigned', 0)
            ->get();
        ...
        foreach ($absentEmployeesByDate as $date => $absentEmployees) {
            foreach ($absentEmployees as $absentEmployee) {
                $absentEmployeesForTable[] = [
                    ...
                    'emp_mobile' => $absentEmployee['emp_mobile'],
                    'emp_address' => $absentEmployee['emp_address'],
