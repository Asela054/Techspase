...
//        }
    ...
    public function employee_list_from_attendance_sel2(Request $request){
        if ($request->ajax())
        {
            ...
            if (!empty($searchTerm)) {
                $query->where('employees.emp_name_with_initial', 'LIKE', '%' . $searchTerm . '%')
                    ->skip($offset)
                    ->take($resultCount);
            } else {
                $query->limit(5); 
            ...
            if (!empty($searchTerm)) {
                $count = DB::query()
                    ...
                    ->where('employees.is_resigned', 0)
                    ->where('employees.status', 1)
                    ->where('employees.emp_name_with_initial', 'LIKE', '%' . $searchTerm . '%')
                    ->select([
                        DB::raw('DISTINCT employees.emp_id as id'),
        ...
        if ($request->ajax())
        {
            ...
            if (!empty($searchTerm)) {
                $count = DB::query()
                    ...
                    ->where('employees.deleted', 0)
                    ->where('employees.status', 1)
                    ->select([DB::raw('DISTINCT employees.emp_id as id')])
                    ->count();
