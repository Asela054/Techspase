...
class OtAllocationController extends Controller
{
    ...
    public function insert(Request $request){
        ...
        $Otallocation->transport = $request->input('transport');
        $Otallocation->meal = $request->input('meal');   
   ...
   public function deletelist(Request $request){
    ...
    public function update(Request $request){
        ...
        foreach ($tableData as $rowtabledata) {
            ...
            if (!empty($rowtabledata['col_3'])) {
                $time_parts = explode(' - ', $rowtabledata['col_3']);
                if (count($time_parts) == 2) {
                    $time_from = trim($time_parts[0]);
                    $time_to = trim($time_parts[1]);
                }
            }
                
            if(isset($rowtabledata['col_4']) && !empty($rowtabledata['col_4'])) {
                $detailID = $rowtabledata['col_4'];
                $Otallocationdetail = Otallocationdetail::find($detailID);
                if ($Otallocationdetail) {
                    $Otallocationdetail->emp_id = $emp_id;
                    $Otallocationdetail->time_from = $time_from;
                    $Otallocationdetail->time_to = $time_to;
    ...
    public function ot_allocate_csv(Request $request)
    {
        ...
        foreach ($fileContents as $line) {
            ...
            // if (!preg_match('/^\d{2}:\d{2}(?::\d{2})?$/', $time_from)) {
            //     $errors[] = "Line {$lineNumber}: Invalid start time format '{$time_from}' - must be HH:MM or HH:MM:SS";
            //     $lineNumber++;
            //     continue;
            // }

            // if (!preg_match('/^\d{2}:\d{2}(?::\d{2})?$/', $time_to)) {
            //     $errors[] = "Line {$lineNumber}: Invalid end time format '{$time_to}' - must be HH:MM or HH:MM:SS";
            //     $lineNumber++;
            //     continue;
            // }
