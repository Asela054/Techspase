...
$columns = array(
	...
	array( 'db' => '`u`.`holiday_normal_hours`', 'dt' => 'holiday_normal_hours', 'field' => 'holiday_normal_hours' ),
	array( 'db' => '`u`.`holiday_double_hours`', 'dt' => 'holiday_double_hours', 'field' => 'holiday_double_hours' ),
	array( 'db' => '`u`.`sunday_work_days`', 'dt' => 'sunday_work_days', 'field' => 'sunday_work_days' ),
	array( 'db' => '`u`.`poya_work_days`', 'dt' => 'poya_work_days', 'field' => 'poya_work_days' ),
	array( 'db' => '`u`.`mercantile_work_days`', 'dt' => 'mercantile_work_days', 'field' => 'mercantile_work_days' ),
	array( 'db' => '`u`.`sunday_double_ot_hrs`', 'dt' => 'sunday_double_ot_hrs', 'field' => 'sunday_double_ot_hrs' ),
	array( 'db' => '`u`.`poya_extended_normal_ot_hrs`', 'dt' => 'poya_extended_normal_ot_hrs', 'field' => 'poya_extended_normal_ot_hrs' ),
...
if(!empty($_POST['department'])){$department = $_POST['department'];}else{$department='';}
if(!empty($_POST['employee'])){$employee = $_POST['employee'];}else{$employee='';}
if(!empty($_POST['location'])){$location = $_POST['location'];}else{$location='';}
if(!empty($_POST['from_date'])){$from_date = $_POST['from_date'];}else{$from_date='';}
if(!empty($_POST['to_date'])){$to_date = $_POST['to_date'];}else{$to_date='';}
...
$joinQuery = "FROM `ot_approved` AS `u` 
	...
	$extraWhere = "1=1";
	$extraWhere .= " AND `emp`.`deleted` = 0";
	$extraWhere .= " AND `emp`.`is_resigned` = 0";
	$extraWhere .= " AND `emp`.`status` = 1";
